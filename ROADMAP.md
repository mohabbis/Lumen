...
- Light outreach across smart-home, Apple, and design communities — the product is niche (neurodivergent-first, consent-before-action), but the audience is not. Anyone who wants a calmer home is welcome.
...
### Decision log — scheduled scenes fire directly (July 2026)

Scene schedules (`Scene.scheduleMinutesSinceMidnight`) run their scene **directly** at the set time, without a per-fire confirmation sheet. This is deliberately **not** a violation of the consent principle above: setting a specific scene to run at a specific time is itself an explicit, ahead-of-time consent — categorically different from the forbidden "auto-apply on a hunch," where Lumen infers intent and acts. To keep it explainable and reversible, every scheduled fire posts a notification ("… ran on schedule — you set this up. Tap to adjust."), and a `grace` window (`ScheduleTiming`) means a missed schedule does not fire stale hours later. Geofence-triggered scenes already followed this same "pre-authorized trigger fires directly + notifies" model. Ambient/inferred suggestions (`SuggestionEngine` → `LumenReasoningView` → `LumenActionView`) remain confirmation-gated and must stay that way.
