...
const screens = [
  ...
  { id: 'control', label: 'Moments', title: 'Evening Wind-Down', subtitle: 'A named moment, not a settings panel.', card: 'Scene', icon: Lightbulb, rows: ['Reading 45% warm', 'Shades 60% closed', 'Temperature 21.5 deg', 'Do not disturb on'], insight: 'Scenes are moods, not presets. "Reading," "Movie Night," "Wind Down" — moments you name once and return to.', action: 'Set the mood' },
  { id: 'reasoning', label: 'Works with', title: 'Works with what you already own', subtitle: 'Govee, GE Cync, Matter, HomeKit — unified.', card: 'Your devices', icon: Brain, rows: ['Govee strip connected', 'GE Cync ceiling linked', 'Matter bridge active', 'HomeKit hub synced'], insight: 'The companion story is the why. Multi-protocol is the how. Stop thinking about which app handles which bulb.', action: 'See your devices' },
  { id: 'action', label: 'Quietly', title: 'Home gets quieter', subtitle: 'Motion slowed. Lumen noticed.', card: 'Scene changes', icon: CheckCircle2, rows: ['Lights easing to 20%', 'Temperature 21.0 deg', 'Notifications paused', 'Scene Wind Down'], insight: 'The home gets smarter quietly — motion, time, and habit. No rule-builder required. Manual control is always close.', action: 'Let it settle' }
];
...
function Waitlist() {
  ...
  return <section className="access" id="access"><div><p className="eyebrow">Early access</p><h2>Your home should know you.</h2><p>Lumen is a calm home companion for iPhone. It notices morning light, evening wind-down, and the habits your day falls into — and responds gently, without you managing it. Works with HomeKit, Matter, Govee, and GE Cync. Everything runs on-device. Nothing leaves your home. Launching 2026.</p><form onSubmit={handleSubmit}><input name="email" type="email" placeholder="Your email address" required/><button disabled={status === 'loading'}>{status === 'loading' ? 'Joining...' : 'Request access'} <ArrowRight size={16}/></button></form><div className="checks"><span>iOS first</span><span>Private beta</span><span>No spam</span></div>{status === 'success' && <p className="form-note">You're on the list — we'll reach out when Lumen is ready.</p>}{status === 'error' && <p className="form-note error">Something went wrong. Try again or email <a href="mailto:m.rafiq2006@icloud.com">m.rafiq2006@icloud.com</a>.</p>}</div><div className="launch-card"><SunMedium size={36}/><b>Launching 2026</b><span>Calm companion · On-device AI · Works with your devices</span></div></section>;
}

export function App() {
  const [menuOpen, setMenuOpen] = useState(false);
  const close = () => setMenuOpen(false);
  const featureStrip = [['Comes home with you','Arrive, and the house already knows.',Waves],['Private by design','Your home. Your data.',LockKeyhole],['On-device intelligence','Fast, local, secure.',Microchip],['Works with your devices','Govee, GE Cync, Matter, HomeKit — all in one place.',Home],['Future ready','Built to evolve with your home.',Infinity]];
