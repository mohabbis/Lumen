...
const screens = [
  { id: 'awareness', label: 'Awareness', title: 'Good evening, Muha', subtitle: 'Your apartment is calm, warm, and secure.', card: 'Favorite rooms', icon: Home, rows: ['Bedroom 21.5 deg', 'Desk lamp ready', 'Kitchen all off', 'Hallway quiet'], insight: 'Bedroom motion faded 18 min ago. Desk lighting can lower now.', action: 'Prepare night mode' },
...
function Waitlist() {
  ...
  return <section className="access" id="access"><div><p className="eyebrow">Private early access</p><h2>Be the first to bring intelligence home.</h2><p>Join the private waitlist and help shape the future of Lumen.</p><form onSubmit={handleSubmit}><input name="email" type="email" placeholder="Your email address" required/><button disabled={status === 'loading'}>{status === 'loading' ? 'Joining...' : 'Join Early Access'} <ArrowRight size={16}/></button></form><div className="checks"><span>No spam</span><span>Private</span><span>Invite only</span></div>{status === 'success' && <p className="form-note">You are on the list. If no backend is configured, your mail app handled the fallback.</p>}{status === 'error' && <p className="form-note error">Submission failed. Configure VITE_WAITLIST_ENDPOINT or Supabase env vars.</p>}</div><div className="launch-card"><SunMedium size={36}/><b>Launching 2026</b><span>Built on Muhome - HomeKit native - Matter ready</span></div></section>;
}
