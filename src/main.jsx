...
function LumenScreen({ step }) {
  if (step.screen === 'Home') {
    return (
      <div className="app-screen">
        <div className="screen-head"><span>My Home</span><b>72°F</b></div>
        <div className="home-hero-card">
          <p className="micro dark">Current state</p>
          <h3>Evening mode is forming.</h3>
          <p>Low motion in the living room. Warm light preference detected after 8:30 PM.</p>
        </div>
        <div className="room-grid">
          <div className="room-tile active"><span>Living</span><b>8 devices</b></div>
          <div className="room-tile"><span>Bedroom</span><b>5 devices</b></div>
          <div className="room-tile"><span>Desk</span><b>3 devices</b></div>
          <div className="room-tile"><span>Entry</span><b>2 sensors</b></div>
        </div>
        <div className="bottom-tabs"><span className="active">Home</span><span>Scenes</span><span>Sense</span></div>
      </div>
    );
  }
  ...
  if (step.screen === 'Room') {
    return (
      <div className="app-screen">
        <div className="screen-head"><span>Living Room</span><b>Ready</b></div>
        <div className="lamp-map">
          <div className="lamp-node large">38%</div>
          <div className="lamp-node">2700K</div>
          <div className="lamp-node">Motion</div>
        </div>
        <div className="device-list">
          <div><span>Walnut lamp</span><b>38%</b></div>
          <div><span>RGBIC strip</span><b>Warm amber</b></div>
          <div><span>Motion sensor</span><b>Quiet</b></div>
        </div>
        <div className="bottom-tabs"><span>Home</span><span className="active">Rooms</span><span>Sense</span></div>
      </div>
    );
  }

  if (step.screen === 'Insight') {
    return (
      <div className="app-screen">
        <div className="screen-head"><span>Insight</span><b>Explain</b></div>
        <div className="insight-card">
          <Brain size={28} />
          <h3>Suggested: Belgravia calm</h3>
          <p>Because motion has slowed, brightness is above your usual evening level, and the room has been occupied for 42 minutes.</p>
        </div>
        <div className="reason-list">
          <div><CheckCircle2 size={15} /><span>Pattern matched from 6 prior evenings</span></div>
          <div><CheckCircle2 size={15} /><span>No sensitive device will run automatically</span></div>
          <div><CheckCircle2 size={15} /><span>HomeKit execution requires confirmation</span></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="app-screen">
      <div className="screen-head"><span>Confirm Scene</span><b>HomeKit</b></div>
      <div className="confirm-panel">
        <Lightbulb size={34} />
        <h3>Apply Belgravia calm?</h3>
        <p>Lumen will dim two lights, warm the strip, and save this as a preference signal.</p>
      </div>
      <div className="action-list">
        <span>Walnut lamp → 38%</span>
        <span>RGBIC strip → warm amber</span>
        <span>Scene memory → update</span>
      </div>
      <button className="app-confirm" type="button">Confirm action</button>
    </div>
  );
}
...
function DeviceMockup({ activeStep = 2 }) {
  ...
  return (
    <motion.div className="device-stage" initial={{ opacity: 0, y: 36, rotate: -2 }} whileInView={{ opacity: 1, y: 0, rotate: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }}>
      <div className="halo-ring" />
      <motion.div className="phone" animate={{ y: [0, -10, 0], rotateZ: [1, 0, 1] }} transition={{ duration: 7, repeat: Infinity, ease: 'easeInOut' }}>
        <div className="screen">
          <div className="status"><span>9:41</span><span className="island" /><span>Lumen</span></div>
          <AnimatePresence mode="wait">
            <motion.div key={current.screen} initial={{ opacity: 0, x: 18, scale: 0.985 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -18, scale: 0.985 }} transition={{ duration: 0.32, ease: 'easeOut' }}>
              <LumenScreen step={current} />
            </motion.div>
          </AnimatePresence>
          <div className="scene-card compact">
            <div className="scene-top"><span>{current.label}</span><span>{current.stat}</span></div>
            <div className="mini-insight"><Icon size={20} /><p>{current.title}</p></div>
          </div>
        </div>
      </motion.div>
    </motion.div>
...
function Hero() {
  return (
    <section id="top" className="hero">
      <motion.div className="hero-copy" initial={{ opacity: 0, y: 34 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.85, ease: [0.22, 1, 0.36, 1] }}>
  ...
  return (
    <section id="architecture" className="section paper">
      ...
      <div className="architecture">
        {architecture.map(([num, title, text], index) => <motion.article className="card" key={title} {...fadeUp} transition={{ ...fadeUp.transition, delay: index * 0.04 }}><span className="num">{num}</span><h3>{title}</h3><p>{text}</p></motion.article>)}
      </div>
    </section>
  );
}
...
function StackSection() {
  return (
    <section id="stack" className="section dark-panel">
      <motion.div className="section-head" {...fadeUp}><p className="eyebrow">Technical stack</p><h2>A luxury surface over a serious system.</h2><p>Each layer has a distinct job: present, model, persist, sync, bridge, observe, suggest, and confirm.</p></motion.div>
      <div className="stack-grid">{stack.map(([k, v], index) => <motion.div className="stack-item" key={k} {...fadeUp} transition={{ ...fadeUp.transition, delay: index * 0.035 }}><b>{k}</b><span>{v}</span></motion.div>)}</div>
    </section>
  );
}
...
function Waitlist() {
  return (
    <section id="access" className="section access">
      <motion.div {...fadeUp}><p className="eyebrow">Early access</p><h2>Follow the Lumen build.</h2><p className="lede">This is the public preview surface while the iOS product and Muhome architecture mature. The form opens a prepared email so the waitlist can be handled manually until the backend is connected.</p></motion.div>
      <motion.form className="signup" action="mailto:Muharafi@umich.edu" method="post" encType="text/plain" {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.12 }}>
        ...
        <label htmlFor="context">What are you building toward?</label><input id="context" name="context" type="text" placeholder="Apartment, house, dorm, test lab..." />
        <button type="submit">Join Early Access</button><p>No payment required. One update when Lumen is ready.</p>
      </motion.form>
    </section>
  );
}
...
export function App() {
  ...
  return (
    <main className="site-shell">
      <Style />
      <AmbientLighting />
      <nav className="nav"><a className="brand" href="#top" aria-label="Lumen home"><span className="brand-mark">L</span><span>Lumen</span></a><div className="nav-links"><a href="#story">Story</a><a href="#architecture">Architecture</a><a href="#stack">Stack</a><a className="nav-cta" href="#access">Early access</a></div></nav>
      <Hero />
